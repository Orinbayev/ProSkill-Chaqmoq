...
async def get_bot_centers_api(admin_tg_id: str):
    url = f"{BACKEND_URL}/hisob/login/bot-centers/"
    params = {"admin_tg_id": admin_tg_id}
    headers = _get_headers()
    async with httpx.AsyncClient() as client:
        try:
...
async def toggle_bot_center_api(admin_tg_id: str, center_id: int, enabled: bool):
    url = f"{BACKEND_URL}/hisob/login/bot-center-toggle/"
    data = {"admin_tg_id": admin_tg_id, "center_id": center_id, "enabled": enabled}
    headers = _get_headers()
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(url, json=data, headers=headers)
